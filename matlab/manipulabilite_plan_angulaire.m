function[xXZ,zXZ,yYZ,zYZ,xXY,yXY,xXZbutee,zXZbutee,yYZbutee,zYZbutee,xXYbutee,yXYbutee,M1,M2,M3,X,Y,Z]=manipulabilite_plan_angulaire(poseDepart,poseFinale, pas, yplanXZ, xplanYZ,zplanXY,courseActionneur,poseInit)
    ...
    for i=1:1:length(X)
        for j=1:1:length(Z)
            ...
            if q1~=9999 && q2~=9999 && q3~=9999  && q4~=9999 
                xXZ=[xXZ;X(i)];
                zXZ=[zXZ;Z(j)];
                M1(j,i)=indice_manipulabilite_angulaire(X(i),yplanXZ,Z(j),poseDepart(4),q1,q2,q3,q4);
    ...
    for i=1:1:length(Y)
        for j=1:1:length(Z)
            ...
            if q1~=9999 && q2~=9999 && q3~=9999  && q4~=9999 
                yYZ=[yYZ;Y(i)];
                zYZ=[zYZ;Z(j)];
                M2(j,i)=indice_manipulabilite_angulaire(xplanYZ,Y(i),Z(j),poseDepart(4),q1,q2,q3,q4);
    ...
    for i=1:1:length(X)
        for j=1:1:length(Y)
            ...
            if q1~=9999 && q2~=9999 && q3~=9999  && q4~=9999 
                xXY=[xXY;X(i)];
                yXY=[yXY;Y(j)];
                M3(j,i)=indice_manipulabilite_angulaire(X(i),Y(j),zplanXY,poseDepart(4),q1,q2,q3,q4);
            else
                M3(j,i)=NaN;
            end
        end
