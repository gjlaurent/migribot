...
for col = 1:3
   ...
   % st=sgtitle([sprintf('Manipulability index for $\\theta=$%.1f deg',k*180/pi)],'FontSize',taillePolice,'Interpreter','latex')%'Angle of the platform ',num2str(k),' rad')
    ...
    plot(xXZ(planXZ),zXZ(planXZ),'Color',[0 0 0.5],'LineWidth',2);
    plot(0,poseinit,'+','Color',[0 0 0.5],'MarkerSize',10,'LineWidth',2);
    plot([-courseactionneur/2 -courseactionneur/2],[-leg 0],'--','Color',[0.5 0 0],'LineWidth',1);
    plot([courseactionneur/2 courseactionneur/2],[-leg 0],'--','Color',[0.5 0 0],'LineWidth',1);
    %plot3(xXZ(planXZ),zXZ(planXZ),altitude,'Color',[0 0 0.5],'LineWidth',2)
    %plot([0 0],[-6 6],'k--');
    %plot([-6 6],[PoseZ PoseZ],'k--');
    axis([-leg/2 leg/2 -leg 0]);
    grid on;
    axis square
    set(s1,'DataAspectRatio',[1 1 100000],'FontSize',taillePolice-8);
    
    title([sprintf('$c=\\frac{%i}{4}l$ and $z_0=\\frac{%i}{4}l$',col, lig)],'FontSize',taillePolice,'Interpreter','latex')
...
savefig([folderName '/workspace.fig']);
