...
folderName ='workspace_manipulabilite_angulaire';
mkdir(folderName);
GifName = 'evolution_tau_theta.gif';
...
for k=debut:(fin-debut)/20:fin
    ...
    PoseI1=[-5.8 -5.8 0 k];
    PoseF1=[5.8 5.8 -6 k];
    [xXZ,zXZ,yYZ,zYZ,xXY,yXY,xXZbutee,zXZbutee,yYZbutee,zYZbutee,xXYbutee,yXYbutee,M1,M2,M3,X,Y,Z]=manipulabilite_plan_angulaire(PoseI1,PoseF1,300,0,0,-5.1,courseactionneur,poseinit);
    %st=sgtitle(['$Manipulability \ index \ for \ the \ angle \ platform \ \theta$'],'Interpreter','latex')
    st=sgtitle([sprintf('$Inverse\\ conditon\\ number \\, \\tau (H)\\ for \\, \\theta=%.2f \\ rad$',k)],'FontSize',taillePolice,'Interpreter','latex')%'Angle of the platform ',num2str(k),' rad')
    ...
    title(['$Projection \ in\ O \mathbf{x_w}\mathbf{z_w}\ with\  y_p=0$'],'FontSize',taillePolice,'Interpreter','latex')
    xlabel(['$x_p \ (mm)$'],'FontSize',taillePolice,'Interpreter','latex')
    ylabel(['$z_p \ (mm)$'],'FontSize',taillePolice,'Interpreter','latex')
    hold off
    ...
    title(['$Projection \ in\ O \mathbf{y_w}\mathbf{z_w}\ with\  x_p=0$'],'FontSize',taillePolice,'Interpreter','latex')
    xlabel(['$y_p \ (mm)$'],'FontSize',taillePolice,'Interpreter','latex')
    ylabel(['$z_p \ (mm)$'],'FontSize',taillePolice,'Interpreter','latex')
    hold off
    ...
    title(['$Projection \ in\ O \mathbf{x_w}\mathbf{y_w}\ with\  z_p=5.1$'],'FontSize',taillePolice,'Interpreter','latex')
    xlabel(['$x_p \ (mm)$'],'FontSize',taillePolice,'Interpreter','latex')
    ylabel(['$y_p \ (mm)$'],'FontSize',taillePolice,'Interpreter','latex')
    hold off
    
   c=colorbar('Location','southoutside','Position',...
    [0.118890765765765,0.176867015340886,0.798141891891892,0.05607813549765]);
    c.Label.String='$\frac{1}{1+\tau} \ (unitless)$';
    c.Label.Interpreter='latex'
    c.Label.FontSize=taillePolice+2;
